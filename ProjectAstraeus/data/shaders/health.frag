uniform int tHealth, tMaxHealth;
...
void main (void)
{
	...
	vec3 tSceneAmbient		= vec3( 0.0, 0.0, 0.0 );
	...
	for ( int iLight = 0; iLight < LIGHT_COUNT; iLight++ ) {
		...
		vec3 tHealthColor = vec3( ( tMaxHealth - tHealth ) / ( tMaxHealth * 2.0 ), tHealth / ( tMaxHealth * 2.0 ), 0.0 );

		tFinalLight += tDiffuse + tSpecular + tHealthColor;
	}

	gl_FragColor = vec4( tFinalLight, 1.0 );
}